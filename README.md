...
Система управления театральной системой с графическим интерфейсом на Python для управления всеми аспектами театральной деятельности.
...
**Eсли установка происходит слишком долго, можете использовать альтернативный подход:**
1. Перед активацией виртуального окружения установите wxPython:
```bash
# Для Linux (Ubuntu/Debian)
sudo apt install python3-wxgtk4.0
```
2. Активируйте виртуальное окружение, создайте символическую ссылку (симлинк) на пакет wxPython и установите все зависимости из requirements.txt, кроме wxPython.
```bash
...
1. Ручная настройка базы данных
```bash
# Импорт структуры базы данных из SQL-файла
mysql -u theater_user -p bd < sql/database_structure.sql
```
2. Проверка структуры базы данных:
```bash

# Подключение к MySQL
mysql -u theater_user -p
...
├── sql/               # SQL-скрипты
│   └── database_structure.sql  # Полная структура БД
...
Система создает файл логов theatre_system.log в корне проекта. Все важные события (ошибки, успешные операции) записываются в этот файл.
Автоматическое обновление

При запуске система проверяет наличие обновлений структуры БД и при необходимости применяет их.
